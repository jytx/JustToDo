// 笔记/任务 导入导出 —— 本地文本文件的用户授权读写
//
// 导入（read_import_text）：读取待导入的文本文件（md/markdown/txt），
// 仅负责安全读取与格式校验（扩展名白名单 + 大小上限），
// Markdown → HTML 的转换在前端完成（marked，与富文本粘贴同一路径），
// 避免在 Rust 侧引入 Markdown 解析依赖。
// 导出（write_export_text）：把任务/笔记导出的 Markdown 写到用户在保存对话框
// 选择的路径（覆盖确认由系统保存面板原生处理）。
// 纯函数式实现：不持有全局状态，路径均来自前端系统文件对话框（用户显式选择）。
...
/// 把导出的 Markdown 文本写入用户选择的路径
///
/// - 路径来自前端系统保存对话框（目录必然存在，无需 create_dir_all）
/// - 同名文件覆盖确认由系统保存面板原生处理，这里直接覆盖写入
/// - 写入失败（权限/磁盘满等）返回 Err，由前端弹错误提示
#[tauri::command]
