...
| 富文本   | **Tiptap 3.x**（StarterKit + CodeBlockLowlight + Image resize + Placeholder + Suggestion + GlobalDragHandle；详见 `discuss/notion-like-editor.md`） |
| 构建工具 | **Vite 6**                                                                  |
...
✅ **笔记本/便签** — 复用 tasks 表（加 `kind` 字段区分 `task`/`note`），笔记本独立成区
